...
| 14    | `camara/despesas_camara.py`              | `camara/parlamentar_camara.py`       | Importa despesas da Cota (CEAP) dos deputados a partir dos **arquivos oficiais anuais** (`camara.leg.br/cotas/Ano-{ano}.csv.zip` — a versão CSV, única que traz o `ideCadastro` do deputado) — independente da API, que está devolvendo vazio |
